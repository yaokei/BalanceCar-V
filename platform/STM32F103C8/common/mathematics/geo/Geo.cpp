...
// lat_0, lon_0 are expected to be in correct format: -> 47.1234567 and not 471234567
int map_projection_init(struct map_projection_reference_s *ref, double lat_0,double lon_0, uint64_t timestamp)
{
	return map_projection_init_timestamped(ref, lat_0, lon_0, timestamp);
}
...
int map_projection_project(const struct map_projection_reference_s *ref, double lat, double lon, float *x, float *y)
{
	...
	*x = k * (ref->cos_lat * sin_lat - ref->sin_lat * cos_lat * cos_d_lon) * M_RADIUS_OF_EARTH;
	*y = k * cos_lat * sin(lon_rad - ref->lon_rad) * M_RADIUS_OF_EARTH;
...
int map_projection_reproject(const struct map_projection_reference_s *ref, float x, float y, double *lat, double *lon)
{
	...
	double x_rad = x / M_RADIUS_OF_EARTH;
	double y_rad = y / M_RADIUS_OF_EARTH;
                                        ...
                                        double *lat_target, double *lon_target)
{
	...
	double radius_ratio = (double)(fabs(dist) / M_RADIUS_OF_EARTH);
...
void get_vector_to_next_waypoint(double lat_now, double lon_now, double lat_next, double lon_next, float *v_n, float *v_e)
{
	...
	/* conscious mix of double and float trig function to maximize speed and efficiency */
	*v_n = M_RADIUS_OF_EARTH * (cos(lat_now_rad) * sin(lat_next_rad) - sin(lat_now_rad) * cos(lat_next_rad) * cos(
			d_lon));
	*v_e = M_RADIUS_OF_EARTH * sin(d_lon) * cos(lat_next_rad);
}
...
void get_vector_to_next_waypoint_fast(double lat_now, double lon_now, double lat_next, double lon_next, float *v_n, float *v_e)
{
	...
	/* conscious mix of double and float trig function to maximize speed and efficiency */
	*v_n = M_RADIUS_OF_EARTH * d_lat;
	*v_e = M_RADIUS_OF_EARTH * d_lon * cos(lat_now_rad);
}
...
void add_vector_to_global_position(double lat_now, double lon_now, float v_n, float v_e, double *lat_res, double *lon_res)
{
	...
	*lat_res = (lat_now_rad + (double)v_n / M_RADIUS_OF_EARTH) * M_RAD_TO_DEG;
	*lon_res = (lon_now_rad + (double)v_e / (M_RADIUS_OF_EARTH * cos(lat_now_rad))) * M_RAD_TO_DEG;
}
                         ...
                         double lat_start, double lon_start, double lat_end, double lon_end)
{
	...
	if (sin(bearing_diff) >= 0) {
		crosstrack_error->bearing = wrap_pi(bearing_track - M_PI_2);
...
int get_distance_to_arc(struct crosstrack_error_s *crosstrack_error, double lat_now, double lon_now,
				 double lat_center, double lon_center, float radius, float arc_start_bearing, float arc_sweep)
{
	...
	if (in_sector) {
		...
		double end_disp_x = (double)radius * sin(wrap_pi((double)(arc_start_bearing + arc_sweep)));
		double end_disp_y = (double)radius * cos(wrap_pi((double)(arc_start_bearing + arc_sweep)));
...
float get_distance_to_point_global_wgs84(double lat_now, double lon_now, float alt_now,
		double lat_next, double lon_next, float alt_next, float *dist_xy, float *dist_z)
{
